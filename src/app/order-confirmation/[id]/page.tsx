  ...
  if (!order) {
    return (
      <div className="max-w-xl mx-auto px-6 py-28 text-center">
        ...
        <Link href="/" className="text-sm text-primary underline underline-offset-4">Return home</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-16 md:py-28">
      <div className="text-center mb-12">
        ...
        <p className="text-xs uppercase tracking-[0.15em] text-zinc-500 font-medium mb-3">Order Confirmed</p>
        <h1 className="text-3xl font-light text-white tracking-tight mb-3">
          Thank you, {order.delivery_name}.
        </h1>
        <p className="text-sm text-zinc-500">
          Order <span className="font-mono text-zinc-400">#{order.id.split('-')[0]}</span> has been placed.
        </p>
      </div>
        ...
        <div className="bg-[#0a0a0a] p-6">
          ...
          <div className="text-sm text-zinc-300 space-y-2">
            ...
            <div className="flex justify-between border-t border-white/[0.04] pt-2 mt-2">
              ...
              <span className="text-white font-medium">Rs. {Number(order.total).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
